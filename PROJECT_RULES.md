...
- 업데이트 규모에 따라 버전 자리수를 다르게 올린다:
  ...
  - 자리올림은 일반적인 버전 표기 규칙을 따른다 (예: 0.0.9 다음 소규모 업데이트 → 0.1.0, 0.9.x 다음 대규모 업데이트 → 1.0.0 등 자연스럽게 올림).
...
2. 파일명 형식: `VX_X_X_영문요약.zip` (⚠️ 한글/괄호/공백 금지 — 실제로 모바일 다운로드 실패 사례 발생함)
   - 버전 번호 + 밑줄(`_`) + 영문(또는 로마자 표기) 요약을 넣는다.
   - 예: `V5_9_9_fix_table_style.zip`
   - 한글, 괄호 `()`, 공백은 절대 사용하지 않는다.
