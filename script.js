var currentSubPage = 0;
        ...
        // ============================================================
        // 🖊️ 본문 S펜 필기: 3색 볼펜 + 3색 형광펜(글씨 투과) + 지우개
        // ============================================================
        function createPenCanvasForPage(page) {
            ...
            function redrawAll() {
                ...
                strokes.forEach(function(s) {
                    ...
                    if (s.erase) {
                        ...
                        ctx.globalCompositeOperation = 'multiply'; // 글씨가 선명하게 비치는 곱하기 합성
                        ctx.strokeStyle = s.color || 'rgba(254, 240, 138, 0.65)';
                        ctx.lineWidth = s.w || 16;
                    } else {
            ...
            var penColor = '#2563eb'; // 파랑(기본), 빨강, 검정
            var highlighterColor = 'rgba(250, 204, 21, 0.65)'; // 노랑(기본), 초록, 분홍

            var toolbar = document.createElement('div');
            toolbar.className = 'unit-pen-toolbar';

            // 1. 도구 선택 버튼 (펜 / 형광펜 / 지우개)
            ...
            var penColors = [
                { color: '#2563eb', bg: '#2563eb' }, // 파랑
                { color: '#dc2626', bg: '#dc2626' }, // 빨강
                { color: '#0f172a', bg: '#0f172a' }  // 검정
            ];
            var highColors = [
                { color: 'rgba(250, 204, 21, 0.65)', bg: '#facc15' }, // 형광 노랑
                { color: 'rgba(74, 222, 128, 0.65)', bg: '#4ade80' }, // 형광 초록
                { color: 'rgba(244, 114, 182, 0.65)', bg: '#f472b6' }  // 형광 분홍
            ];
            ...
            wrap.addEventListener('pointerdown', function(e) {
                ...
                if (e.target && e.target.closest && e.target.closest('.unit-pen-toolbar')) return;
                ...
                var widthVal = isEraser ? 24 : (isHigh ? 16 : (1.2 + (e.pressure || 0.5) * 2.5));
                ...
                if (isEraser) {
                    ...
                    ctx.globalCompositeOperation = 'multiply'; // 글자 비침
                } else {
                    ctx.globalCompositeOperation = 'source-over';
                }
