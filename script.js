var currentSubPage = 0;
        ...
        // 🌟 전역 스크립트 에러 감지 UI 토스트
        window.addEventListener('error', function(e) {
            if (e && e.message) {
                showAppToast('⚠️ 시스템 알림: ' + e.message, 3000);
            }
        });
        ...
        function createPenCanvasForPage(page) {
            ...
            function checkStorageSafety() {
                ...
                for (var key in localStorage) {
                    if (localStorage.hasOwnProperty(key)) {
                        totalBytes += (localStorage[key].length + key.length) * 2; // UTF-16 고려
                    }
                }
                // 브라우저 한도 5MB 중 80%(약 4MB) 초과 시 사전 경고
                if (totalBytes > 4 * 1024 * 1024) {
                    showAppToast('⚠️ 필기 저장 공간이 거의 찼어요(80% 이상). 상단 백업 버튼을 눌러 보관해두세요!', 3500);
                }
            }
            ...
            function persistStrokes() {
                try {
                    ...
                    showAppToast('🚨 저장 공간이 부족해 필기가 저장되지 않았어요! 상단 백업 후 불필요한 필기를 비워주세요.', 3500);
                }
            }
